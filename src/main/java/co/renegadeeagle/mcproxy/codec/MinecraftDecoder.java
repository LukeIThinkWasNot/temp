...
public class MinecraftDecoder extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(final ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (socketState == null) {
            ...
            if (packetID == 0) {
                ...
                Node givenNode = nodeFromHostname(hostname);
                if (givenNode == null) {
                    System.err.println("Could not find any node with Hostname: " + hostname);
                    return;
                }

                System.out.println("Got node with port: " + givenNode.getRemoteHostPort() );
                final ChannelFuture cf = b.connect(givenNode.getRemoteHostname(), givenNode.getRemoteHostPort());
                ...
                cf.addListener(new ChannelFutureListener() {
                    ...
                    public void operationComplete(ChannelFuture future) throws Exception {
                        if (future.isSuccess()) {
                            ...
                            System.out.println("Failed to connect to " + future);
    ...
    public Node nodeFromHostname(String req) {
        for (Node node : Main.getSettings().getNodes()) {
            System.out.println("Expected: " + req + " | Hostname: " + node.getHostname() + " Remote host name " + node.getRemoteHostname());
            System.out.println(req + " = " + node.getHostname());
            System.out.println(node.getHostname().trim().equalsIgnoreCase(req.trim()));
            if (node.getHostname().trim().equalsIgnoreCase(req.trim())) {
                System.out.println("Found node: " + node.getHostname());
                return node;
            }
        }
        return null;
